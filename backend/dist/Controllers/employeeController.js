...
exports.getAnalytics = exports.deleteEmployee = exports.updateEmployee = exports.createEmployee = exports.getEmployeeById = exports.getEmployees = void 0;
...
const getEmployees = async (req, res) => {
    try {
        ...
        const [employees, total] = await Promise.all([
            employeeModel_1.default.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
            employeeModel_1.default.countDocuments(filter),
        ]);
...
const getEmployeeById = async (req, res) => {
    try {
        const employee = await employeeModel_1.default.findById(req.params.id).lean();
        if (!employee) {
...
const createEmployee = async (req, res) => {
    try {
        ...
        const existing = await employeeModel_1.default.findOne({ email: email.trim().toLowerCase() });
        if (existing) {
...
const updateEmployee = async (req, res) => {
    try {
        ...
        if (email && email.trim().toLowerCase() !== employee.email) {
            const duplicate = await employeeModel_1.default.findOne({
                email: email.trim().toLowerCase(),
                _id: { $ne: employee._id },
            });
            if (duplicate) {
...
const deleteEmployee = async (req, res) => {
    try {
        const employee = await employeeModel_1.default.findByIdAndDelete(req.params.id);
        if (!employee) {
            return res.status(404).json({ message: "Employee not found" });
        }
...
const getAnalytics = async (_req, res) => {
    try {
        const [totalEmployees, activeEmployees, departmentWise, statusDistribution, monthlyJoined] = await Promise.all([
            employeeModel_1.default.countDocuments(),
            employeeModel_1.default.countDocuments({ status: "Active" }),
            employeeModel_1.default.aggregate([
